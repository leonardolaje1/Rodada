...
function parseGPX(xml) {
  ...
  for (let i = 1; i < puntos.length; i++) {
    const a = puntos[i - 1]
    const b = puntos[i]
    if (a.lat && b.lat) km += haversineKm(a.lat, a.lon, b.lat, b.lon)
    if (a.ele != null && b.ele != null && b.ele > a.ele) desnivel += b.ele - a.ele
  }
...
function parseTCX(xml) {
  ...
  for (let i = 1; i < alturas.length; i++) {
    if (alturas[i] > alturas[i - 1]) desnivel += alturas[i] - alturas[i - 1]
  }
